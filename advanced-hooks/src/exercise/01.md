...
`useReducer` is an alternative to useState if we need to update multiple state 
values at the same time, or if we want to separate the logic for the state from 
the component that use the state or updates it. useReducer accepts three arguments:
1. a reducer function: `const reducer = (state, newState) => newState`
2. an initial argument
3. an optional lazy initialization function: helpful for avoiding expensive calculations
on rerenders
```javascript
const [username, setUsername] = React.useReducer(
  (username, newUsername) => newUserName,
  'Noone',
  () => window.localStorage.getItem('username') || ''
)
```
The above is a simplified implementation of setState, using `useReducer`. The conventional 
implementation though not always necessary implementation of useReducer utilizes an
action/dispatch object with a type and a switch statement.
```javascript
...
function NameInput({initialName = 'Noone'}) {
  const [state, dispatch] = React.useReducer(
    nameReducer
    initialName,
    (name) => window.localStorage.getItem('username') || name // name === initialName
  );

  return (
    <input 
      value={state.name} 
      onChange={() => dispatch({ type: 'UPDATE_NAME', userName: e.target.value })}/>
  )
}
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=kevin.s.kabore%40gmail.com).
