...
the component that use the state or updates it. useReducer accepts three
arguments:

1. a reducer function: `const reducer = (state, newState) => newState`
2. an initial argument
3. an optional lazy initialization function: helpful for avoiding expensive
   calculations on rerenders
...
The above is a simplified implementation of setState, using `useReducer`. The
conventional implementation though not always necessary implementation of
useReducer utilizes an action/dispatch object with a type and a switch
statement.
