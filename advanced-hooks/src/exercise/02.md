...
`useCallback` returns a memoized callback function, and provides a dependency array
to specify when this function should be redefined. Every value referenced in the
callback should be listed in the dependency array.
```javascript
const getDiffMemo = React.useCallback(() => getDiff(a, b), [a, b])
```
`useMemo` is similar, except it applies memoization to any type, not functions only.
This allows us to only redefine some variable only when necessary instead of on every
re-render.
```javascript
...
Referential equality is a problem if you want to specify an object/array/function 
as a dependency. It will nevery be equal so this will always re-run. This example
illustrates exactly how you would fix this:
```javascript
