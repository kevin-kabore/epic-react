...
useDebugValue is a hook provided for debugging custom hooks with the React DevTools.
`React.useDebugValue(value, formatFn)` accepts a value and an optional format function as 
a second argument.

```javascript
React.useDebugValue('Bye felicia')
```
or
```javascript
const formatDebugVal = ({count, step}) => `step: ${step}, count: ${count}`
function useCount({initialCount = 0, step = 1} = {}) {
  const [count, setCount] = React.useState(0)
  React.useDebugValue({count, step}, formatDebugVal)
  const increment = () => setCount(c => c + 1)
  return [count, increment]
}
```
Whatever you like, but the format function is mostly helpful if we need to derive the value using some computationally expensive functions. Since this is a debugging hook, our users don't benefit from this and we're "hurting" performance by creating the object and the function. I say "hurting" because it's so fast that it doesn't really matter but something to keep in mind. Every time our custom hook is called in a component, if we check the dev tools we're now able to see the debug values we specified.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=06%3A%20useDebugValue%3A%20useMedia&em=kevin.s.kabore%40gmail.com).
