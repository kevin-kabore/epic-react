...
Context Module Functions pretty much involve wrapping our dispatches in a
function to simplify the use of our contexts. If we leave it up to the user they
might miss a dispatch in a series of dispatches, so we accept a dispatch and
expose a function that can be called. That way we can ensure that the dispatches
are called in the right order for a particular action.

Another approach is to put the logic for our dispatch type in the consuming hook
or the `value` in the `Provider` function. But that has certain drawbacks where
we can't tree shake, code split, and lazy load. Also, our users would haveto
worry about memoizing stuff just to list them in a useEffect or useCallback
dependency array.
...
- Context Module functions when we need to make multiple dispatches ex: async
  calls with status/error
