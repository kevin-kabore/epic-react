...
## 📝 Your Notes
Context Module Functions pretty much involves wrapping our dispatches in a function to
simplify the use of our contexts. If we leave it up to the user they might miss a dispatch
in a series of dispatches, so we accept a dispatch and expose a function that can be called. 
That way we can ensure that the dispatches are called in the right order for a particular
action.

Another approach is to put the logic for our dispatch type in the consuming hook or the 
`value` in the `Provider` function. But that has certain drawbacks where we can't tree shake,
code split, and lazy load. Also, our users would haveto worry about memoizing stuff 
just to list them in a useEffect or useCallback dependency array.

So the typical pattern is: 
- Expose the state and dispatch within Context
- The dispatch will be used directly by consumers
- Context Module functions when we need to make multiple dispatches ex: async calls with status/error
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=01%3A%20Context%20Module%20Functions&em=kevin.s.kabore%40gmail.com).
