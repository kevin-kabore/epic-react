...
Compound components are components that are used together to form a complete UI element.
...
```
Where `Tabs`, `TabList`, and `TabPanels` could be compound components that share implicit state. That 
way our users don't have to worry about managing the state and instead just use our declarative components.

To create a compound component, we use a combination of `React.Children.map` and `React.cloneElement` to forward
our porps. We cannot clone DOM elements, so instead we just return those in the map if the type is a string.
