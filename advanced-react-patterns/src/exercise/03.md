...
When we use `React.Children.map` with `React.cloneElement`, we can only pass
props and clone the immediate children. Another method of passing props that
gets us around this is to use `React.createContext` where the provider and
children are returned from the parent.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=03%3A%20Flexible%20Compound%20Components&em=kevin.s.kabore%40gmail.com).
