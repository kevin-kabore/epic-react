...
When we use `React.Children.map` with `React.cloneElement`, we can only pass props and clone
the immediate children. Another method of passing props that gets us around this
is to use `React.createContext` where the provider and children are returned from
the parent.
