...
The state reducer pattern enables us to give users of our components the ability
to expose and manage the internal state of our application. It works by giving
the user "inversion of control" over our state reducer. This pattern allows them
to manage what state changes happen when a state change happens.

Typically we'll do this by:

1. Defining a reducer to manage our state,
2. Accepting a default reducer as a prop to our custom hook
3. defaulting the reducer in step 2 to our reducer in step 1
4. defining `actionTypes`, and exporting them along with our reducer
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=05%3A%20State%20Reducer&em=kevin.s.kabore%40gmail.com).
